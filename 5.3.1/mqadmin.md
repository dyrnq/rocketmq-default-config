...
```bash
usage: mqadmin rocksDBConfigToJson [-h] [-n <arg>] -p <arg> -t <arg>
 -h,--help                Print help
 -n,--namesrvAddr <arg>   Name server address list, eg: '192.168.0.1:9876;192.168.0.2:9876'
 -p,--configPath <arg>    Absolute path to the metadata config directory
 -t,--configType <arg>    Name of kv config, e.g. topics/subscriptionGroups/consumerOffsets
```

## checkRocksdbCqWriteProgress

```bash
usage: mqadmin checkRocksdbCqWriteProgress -c <arg> [-h] -n <arg> [-t <arg>]
 -c,--cluster <arg>          cluster name
 -h,--help                   Print help
